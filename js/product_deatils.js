$(function() {
	...
	$(".product-wrapper .link").on('click', function(event) {
		...
		$(".product-quick-view.filter").slideDown('slow');
	...
	$('.product-wrapper .link').on('click',function(){
		...
		var selectCanvas2 = jQuery(this).closest('.product-wrapper').children('.portfolio_details').children('.product-quick-view');
		...
		jQuery(this).closest('.product-wrapper').children('.portfolio_details').load('product.html',data,function(res){
